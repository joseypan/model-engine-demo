...
import { initSelectionBox } from "./selectionBox";
// import config from "@/config";
/*
 * 描述：渲染器实例
 */
export let renderer: typeof WebGLRenderer | null;
/*
 * 描述：声明场景对象
 */
export let scene: typeof Scene | null;
/*
 * 描述：用来声明相机变量
 */
export let camera: typeof PerspectiveCamera | null;
/*
 * 描述：控制器对象
 */
...
const closeControlsRotate = () => {
  if (!controls) return;
  controls.enableRotate = false;
};

const openControlsRotate = () => {
  if (!controls) return;
...
// const modelC =
//   "http://61.177.20.107:8000/gtu3d/modelExport/%E5%8F%B0%E7%9B%86%E8%8A%82%E7%82%B9.gltf";
// const modelD = "model.glb";
// const modelE = "http://61.177.20.107:8000/gtu3d/modelExport/test23.gltf";
// const modelF = "http://61.177.20.107:8000/gtu3d/modelExport/JCZX-2AR(6).gltf";
...
const initModel = (
  ...
  return new Promise((resolve, reject) => {
    ...
    // 这里需要清空modelGroup里面所有的内容
    // 需要把之前的删除
    const rootModel = scene.getObjectByName("model-root");
    console.log("rootModel", rootModel);
    if (rootModel) {
      // 清空modelGroup里面所有的内容
      rootModel.remove(...rootModel.children);
      // scene.remove(rootModel);
      rootModel.removeFromParent();
    }
    const modelGroup: typeof Group = new THREE.Group();
    modelGroup.name = "model-root";
    ...
    // }
    //step5:尝试引入模型
    const modelPromise: Promise<any>[] = [];
    modelPathList.forEach((ele) => {
      modelPromise.push(loadModel(ele, modelGroup));
    });
    Promise.all(modelPromise).then((res) => {
      console.log("modelGroup", modelGroup);
      // if (modelGroup.children.length > 0) {
      getFitScaleValue(modelGroup);
      setModelPosition(modelGroup);
      // adjustCameraToFitModel(modelGroup);
      // }
...
const loadModel = (modelPath: string, modelGroup: typeof Group) => {
  return new Promise((resolve, reject) => {
    loader.load(
      modelPath,
      (model) => {
        ...
        model.scene.traverse((obj: any) => {
          if (obj.type === "Mesh") {
            ...
            //   opacity: 0.5, // 设置材质的透明度
            //   side: THREE.FrontSide, // 设置为双面显示
            //   reflectivity: 1, // 设置反射光强度
            //   refractionRatio: 0.78, // 设置折射率
...
export const drawModelView = (
  ...
  return new Promise((resolve, reject) => {
    ...
    initLabel(canvas);
    loadEnviroment();
    initModel(modelPathList, canvas, onProgress).then(resolve).catch(reject);
    container = canvas;
  });
};
  ...
  group.traverse(function (obj3d: typeof Object3D) {
    if (obj3d.type === "Mesh") {
      ...
      if (obj3d.userData.ElementID) {
        ...
        meshPositionMap.set(obj3d.userData.ElementID, position);
      }
    }
  });
  console.log("meshPositionMap", meshPositionMap);
};
...
/**
 * 描述：根据二维屏幕坐标转换成3d中的坐标
 * @param {  }
 * @return
 */
export const get3DPosByCanvasPos = (pos: typeof Vector2) => {
  // const vec = new THREE.Vector3(); // create once and reuse
  // const target = new THREE.Vector3(); // create once and reuse
  // vec.set(
  ...
  // return target;
  // console.log("scene", scene);
  const pX = (pos.x / container.clientWidth) * 2 - 1;
  const pY = (pos.y / container.clientHeight) * 2 - 1;
  // console.log("xy", pX, pY);
  return { x: pX, y: pY };
};
